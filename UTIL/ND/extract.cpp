...
BOOL ExtractUnquotedGSMHexStringToUnicode(const BYTE* szIn, UINT32 cbInLen,  WCHAR* wszOut, UINT32 cbOutLen)
{
    ...
    if ((!szIn) || (0 == cbInLen))
    {
        // No input provided, bail out
        goto Error;
    }

    // First figure out what encoding is being used by examining the first byte
    bEnc = *szInWalk++;
    ...
    switch(bEnc)
    {
        ...
        case 0x82:
        {
            ...
            // GSM Mixed with code page and length
            break;
        }
        default:
        {
            // GSM default chars
            if(RRIL_E_NO_ERROR == GSMDefaultHexToUnicode(szIn, cbInLen, wszOut, cbOutLen / sizeof(WCHAR)))
            {
                fRet = TRUE;
            }
...
BOOL ExtractQuotedGSMHexStringToUnicode(const BYTE* szStart,  WCHAR* wszOut, UINT32 cbOutLen, const BYTE* &rszEnd)
{
    ...
    if (SkipString(szWalk, "\"", szWalk))
    {
        ...
        if (FindAndSkipString(szWalk, "\"", szWalk))
        {
            UINT32 nInLen = szWalk - szTemp - 1;

            if (ExtractUnquotedGSMHexStringToUnicode(szTemp, nInLen, wszOut, cbOutLen))
            {
                fRet = TRUE;
                rszEnd = szWalk;
            }
...
BOOL ExtractQuotedUnicodeHexStringToUnicode(const BYTE* szStart,  WCHAR* wszOut, UINT32 cbOutLen, const BYTE* &rszEnd)
{
    ...
    if (SkipString(szWalk, "\"", szWalk))
    {
        ...
        if (FindAndSkipString(szWalk, "\"", szWalk))
        {
            ...
            if (ExtractUnquotedUnicodeHexStringToUnicode(szTemp, nInLen, wszOut, cbOutLen))
            {
                fRet = TRUE;
                rszEnd = szWalk;
            }
...
BOOL ConvertSizeToUnicodeSize(ENCODING_TYPE enc, size_t cbIn, size_t &cbOut)
{
    ...
    case ENCODING_GSMDEFAULT_UTF8:
        // 1 byte of data maps to 2-bytes of unicode
        cbOut = cbIn << 1;
        return TRUE;

    case ENCODING_GSMDEFAULT_HEX:
        // 2 bytes of hex data map to 2-bytes of unicode
        cbOut = cbIn;
        return TRUE;

    case ENCODING_GSMDEFAULT_UNICODE:
        // 4 characters map to 2-bytes of unicode
        // assumes cbIn is even, which it must
        cbOut = cbIn >> 1;
        return TRUE;
    default:
        return FALSE;
