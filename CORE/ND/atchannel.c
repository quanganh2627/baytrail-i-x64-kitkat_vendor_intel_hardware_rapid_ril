...
static void at_response_free_nolock(ATResponse *p_response);
...
{
    if (sp_response != NULL) {
        at_response_free_nolock(sp_response);
    }
...
{
    ATResponse *ptr;
    ptr = calloc(1, sizeof(ATResponse));
    if ( ptr == NULL) {
        return NULL;
    }
    memset(ptr, 0x0, sizeof(ATResponse));
    return ptr;
}
...
{
    pthread_mutex_lock(&s_commandmutex);
    at_response_free_nolock(p_response);
    pthread_mutex_unlock(&s_commandmutex);
}
