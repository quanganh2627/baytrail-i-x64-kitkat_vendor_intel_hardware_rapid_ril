...
//  All URCs go on this channel.
INITSTRING_DATA URCBasicInitString   = { "E0V1Q0X4|S0=0|+CMEE=1|+XSIMSTATE=1|+XCALLSTAT=1|+CTZU=1|+CTZR=1|+XREG=1|+CREG=2|+CGREG=2|+CGEREP=1,0|+CSSN=1,1|+CMGF=0" };
INITSTRING_DATA URCUnlockInitString  = { "+CRC=1|+CCWA=1|+CUSD=1" };
...
BOOL CChannel_URC::AddSilos()
{
    ...
    {
        RIL_LOG_CRITICAL("CChannel_URC::RegisterSilos : ERROR : chnl=[%d] Could not add CSilo_Voice\r\n", m_uiRilChannel);
        goto Error;
    }
    ...
    if (!pSilo || !AddSilo(pSilo))
    {
        RIL_LOG_CRITICAL("CChannel_URC::RegisterSilos : ERROR : chnl=[%d] Could not add CSilo_Network\r\n", m_uiRilChannel);
        goto Error;
    }
    ...
    if (!pSilo || !AddSilo(pSilo))
    {
        RIL_LOG_CRITICAL("CChannel_URC::RegisterSilos : ERROR : chnl=[%d] Could not add CSilo_SMS\r\n", m_uiRilChannel);
        goto Error;
    }
    ...
    if (!pSilo || !AddSilo(pSilo))
    {
        RIL_LOG_CRITICAL("CChannel_URC::RegisterSilos : ERROR : chnl=[%d] Could not add CSilo_SIM\r\n", m_uiRilChannel);
        goto Error;
    }
    ...
    if (!pSilo || !AddSilo(pSilo))
    {
        RIL_LOG_CRITICAL("CChannel_URC::RegisterSilos : ERROR : chnl=[%d] Could not add CSilo_Phonebook\r\n", m_uiRilChannel);
        goto Error;
    }
